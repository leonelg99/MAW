...
inline void initPWM2(uint8_t freq) {
    ...
    TCCR2A |= BV(COM2B1) | BV(WGM20) | BV(WGM21); // fast PWM, turn on oc2b
    TCCR2B = freq & 0x7;
    OCR2B = 0;
...
MS_Stepper::MS_Stepper(uint16_t steps, uint8_t num) {
  ...
  if (steppernum == 1) {
    latch_state &= ~BV(MOTOR1_A) & ~BV(MOTOR1_B) &
      ~BV(MOTOR2_A) & ~BV(MOTOR2_B); // all motor pins to 0
    MC.latch_tx();
    ...
    latch_state &= ~BV(MOTOR3_A) & ~BV(MOTOR3_B) &
      ~BV(MOTOR4_A) & ~BV(MOTOR4_B); // all motor pins to 0
    MC.latch_tx();
